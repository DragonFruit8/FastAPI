...
from utils.security import Password, create_access_token, get_current_user
...
async def signup(user: UserCreate):
    ...
    return JSONResponse(
        status_code=status.HTTP_201_CREATED , content={"msg": "User created successfully", "email": new_user.email, "created_at": new_user.created_at}
    )
...
@router.get("/me")
async def dashboard(user: UserOut = Depends(get_current_user)):
    return user

...
async def signup(user: UserCreate):
    ...
    return JSONResponse(
        status_code=status.HTTP_201_CREATED , content=UserOut(**new_user.email, created_at=new_user.created_at, Password
    )
    
@router.post("/access-token")
async def get_access_token(form_data: OAuth2PasswordRequestForm = Depends()):    user = await User.authenticate(

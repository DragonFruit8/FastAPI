...
from utils.security import Password, create_access_token, get_current_active_user
...
async def signup(user: UserCreate):
    ...
    return JSONResponse(
        status_code=status.HTTP_201_CREATED , content=UserOut(**new_user.email)
    )
...
async def dashboard(user: UserOut = Depends(get_current_active_user)):
    return user
    # return JSONResponse(
    #     status_code=status.HTTP_200_OK, content=UserOut(**user.dict())
    # )
